...
(define-map notification-subscriptions
  { doc-id: uint, subscriber: principal }
  {
    active: bool,
    created-at: uint,
    updated-at: uint
...
(define-map events-by-doc-recipient-index
  { doc-id: uint, recipient: principal, seq: uint }
  { id: uint }
)

(define-map event-counters-by-doc-recipient
  { doc-id: uint, recipient: principal }
  { count: uint }
)

(define-map last-event-block
  { doc-id: uint, recipient: principal, n-type: uint }
  { height: uint }
)
...
(define-map subs-by-doc-index
  { doc-id: uint, seq: uint }
  { subscriber: principal }
)

(define-private (is-valid-type (t uint))
  (or (is-eq t NOTIF-TYPE-EMAIL)
      (or (is-eq t NOTIF-TYPE-STATUS)
...
(define-read-only (is-subscribed (doc-id uint) (who principal))
  (match (map-get? notification-subscriptions { doc-id: doc-id, subscriber: who })
    s (get active s)
    false
  )
)
      ...
      some-sub
        (if (get active some-sub)
          ...
          (begin
            ...
            (let
              (
                (sc (default-to { count: u0 } (map-get? subs-count-by-doc { doc-id: doc-id })))
                (next (+ (get count sc) u1))
              )
              (map-set subs-count-by-doc { doc-id: doc-id } { count: next })
              (map-set subs-by-doc-index { doc-id: doc-id, seq: next } { subscriber: tx-sender })
            )
            (ok true)
          )
        )
...
(define-public (send-notification (recipient principal) (doc-id uint) (n-type uint))
  (let
    (
      (now stacks-block-height)
    ...
    (if (not (is-valid-type n-type))
      ...
      (if (not (is-subscribed doc-id recipient))
        ...
        (let
          (
            (allowed-type
              (if (is-eq n-type NOTIF-TYPE-EMAIL)
          ...
          (if (not allowed-type)
            ...
            (if (and (not (is-eq tx-sender recipient)) (not allow-ext))
              ...
              (let
                (
                  (last (map-get? last-event-block { doc-id: doc-id, recipient: recipient, n-type: n-type }))
                )
                (if (and (is-some last) (< (- now (get height (unwrap-panic last))) min-gap))
                  ...
                  (let
                    (
                      ...
                      (erc (default-to { count: u0 } (map-get? event-counters-by-doc-recipient { doc-id: doc-id, recipient: recipient })))
                    ...
                    (begin
                      ...
                      (map-set event-counters-by-doc-recipient { doc-id: doc-id, recipient: recipient } { count: next-seq })
                      (map-set events-by-doc-recipient-index { doc-id: doc-id, recipient: recipient, seq: next-seq } { id: next-id })
                      (map-set last-event-block { doc-id: doc-id, recipient: recipient, n-type: n-type } { height: now })
                      (ok next-id)
                    )
                  )
                )
...
(define-public (ack-notification (event-id uint))
  (let
    (
      (e (map-get? notification-events { id: event-id }))
    )
    (match e
      ev
        (if (not (is-eq (get recipient ev) tx-sender))
          (err ERR-UNAUTHORIZED)
          (begin
...
(define-read-only (get-subscription (doc-id uint) (who principal))
  (map-get? notification-subscriptions { doc-id: doc-id, subscriber: who })
)
...
(define-read-only (get-event-id-by-doc-recipient (doc-id uint) (recipient principal) (seq uint))
  (map-get? events-by-doc-recipient-index { doc-id: doc-id, recipient: recipient, seq: seq })
)

(define-read-only (get-event-count (doc-id uint) (recipient principal))
  (get count (default-to { count: u0 } (map-get? event-counters-by-doc-recipient { doc-id: doc-id, recipient: recipient })))
)

(define-read-only (get-subscriber-by-doc-and-seq (doc-id uint) (seq uint))
  (map-get? subs-by-doc-index { doc-id: doc-id, seq: seq })
)

(define-read-only (get-subs-count-for-doc (doc-id uint))
  (get count (default-to { count: u0 } (map-get? subs-count-by-doc { doc-id: doc-id })))
)

(define-read-only (get-last-event-block (doc-id uint) (recipient principal) (n-type uint))
  (map-get? last-event-block { doc-id: doc-id, recipient: recipient, n-type: n-type })
)
